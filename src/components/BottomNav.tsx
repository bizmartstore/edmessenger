...
import { MessageCircle, BookOpen, ClipboardList, Home, FolderKanban, Lightbulb } from "lucide-react";
...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-40 safe-bottom pb-2 px-2 pointer-events-none">
      <div className="glass-card pointer-events-auto max-w-md mx-auto rounded-3xl px-0.5 py-1.5 flex items-center justify-between">
        {items.map(({ to, label, icon: Icon, exact, badgeKey }) => {
          ...
          return (
            <Link
              ...
              to={to}
              className={cn(
                "relative flex-1 flex flex-col items-center gap-0.5 py-1.5 rounded-2xl transition-all duration-200",
                active ? "text-primary-foreground gradient-primary shadow-glow" : "text-muted-foreground hover:text-foreground"
              )}
            >
              <span className="relative">
                <Icon className={cn("h-4 w-4 transition-transform", active && "scale-110")} strokeWidth={active ? 2.5 : 2} />
                {!active && <UnreadBadge count={count} />}
              </span>
              <span className={cn("text-[8px] font-medium tracking-wide", active && "font-semibold")}>{label}</span>
            </Link>
          );
        })}
      </div>
